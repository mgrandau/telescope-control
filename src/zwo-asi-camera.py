...
class ZwoAsiCamera(IImageAcquisitionBehavior):
    ...
    def __init__(
        ...
        automatically_run_post_processing_behaviors: List[
            IPostProcessingBehavior
        ] = None,
        logger: IImageAcquisitionBehaviorLogger = None,
    ):
        self._repository = ZwoAsiCameraDataRepository(configuration)
        self._automatically_run_post_processing_behaviors = (
            automatically_run_post_processing_behaviors
        )
        self._logger = logger

        if self._repository.use_hardware:
            self._video = asi.Camera(self._repository.index)
            self._video.setCaptureFrameFormat(
                self._repository.width,
                self._repository.height,
                1,
                self._repository.mode,
            )
            self._video.setControlValueManual("ASI_GAIN", self._repository.gain)
            self._video.setControlValueManual(
                "ASI_WB_R", self._repository.white_balance_red
            )
            self._video.setControlValueManual(
                "ASI_WB_B", self._repository.white_balance_blue
            )
    ...
    def _wait_for_next_frame(self) -> None:
        ...
        if wait_time > 0:
            if self._logger is not None:
                self._logger.log_frame_acquisition_wait(
                    f"Waiting for next frame: {wait_time}"
                )
            sleep(wait_time)
    ...
    def _get_frame(self) -> None:
        if self._repository.use_hardware:
            ...
            if self._logger is not None:
                self._logger.log_frame_acquisition(
                    f"Acquired frame from camera({self._repository.index})"
                )
        else:
            self._last_image = cv2.imread(self._repository.override_filename)
            if self._logger is not None:
                self._logger.log_frame_acquisition(
                    f"Acquired frame from file : {self._repository.override_filename}"
                )

        self._repository.time_to_get_next_frame = time_ns() + int(
            (1 / self._repository.fps) * 1e9
        )

    def _run_post_processing_behaviors(self) -> None:
        for behavior in self._repository.automatically_run_post_processing_behaviors:
            if self._logger is not None:
                self._logger.log_post_processor_run(
                    f"Before post processor: {type(behavior)}"
                )
            ...
            if self._logger is not None:
                self._logger.log_post_processor_run(
                    f"After post processor: {type(behavior)}"
                )
