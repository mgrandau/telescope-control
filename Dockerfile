...
# and `pip install telescope-control` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "telescope-control", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n telescope-control -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "telescope-control", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "telescope-control", "YOUR_CONSOLE_SCRIPT"]
