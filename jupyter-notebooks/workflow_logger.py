...
class WorkflowLogger:
    ...
    def __init__(self, step : str = '', workflow_log : pd.DataFrame = pd.DataFrame(columns=['Time','Step','Comment'])) -> None:
        self._step = step
        self._workflow_log = workflow_log
        self._done_button = widgets.Button(description='Done')
        self._comment_input = widgets.Textarea(description='Comments:', value = '', PlaceHolder='Enter comment here')
    ...
    def _on_done_button_clicked(self, *args):
        self._workflow_log.loc[len(self._workflow_log.index)] = [datetime.now(), self._step, self._comment_input.value] 
